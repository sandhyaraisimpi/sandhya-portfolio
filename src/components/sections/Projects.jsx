...
//                         {Array.from({length:Math.max(0, filterProjects.length -2)}).map((_, index)=>(
//                             <button 
//                                 key={index}
...
import React, { useState, useRef } from 'react'
import { projects, categories } from '../../data/project' // Ensure this file matches the data we updated
import { Briefcase, Target, Globe, Palette, Zap, ChevronLeft, ChevronRight, BrainCircuit } from 'lucide-react'
import Projectcard from '../ui/Projectcard'
import FadeIn from '../animations/FadeIn'
...
const Projects = () => {
    ...
    const scrollToIndex = (index) => {
        ...
        if (scrollContainerRef.current) {
            const container = scrollContainerRef.current;
            // Calculate width dynamically based on container size
            const cardWidth = container.scrollWidth / filterProjects.length;
            container.scrollTo({
                left: cardWidth * index,
                behavior: 'smooth'
            });
    ...
    // Category icons mapping - Added 'AI & Research' to match your data
    const categoryIcons = {
        'All': Target,
        'Web Apps': Globe,
        'UI Components': Palette,
        'Full Stack': Zap,
        'AI & Research': BrainCircuit, // Icon for your QCSO and Deep Learning research
    };
            ...
            <div className='relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8'>
                <FadeIn delay={0}>
                    <div className='text-center mb-12'>
                        ...
                        <p className='text-lg text-white/60 max-w-2xl mx-auto'>
                            Showcasing my full-stack development and AI research achievements.
                        </p>
                    </div>
                </FadeIn>
                    ...
                    <div className='flex flex-wrap justify-center gap-3 mb-16'>
                        {categories.map((category) => (
                            ...
                            >
                                ...
                                <div className='relative flex items-center gap-2'>
                                    {/* Dynamically render the icon for each category */}
                                    {categoryIcons[category] && React.createElement(categoryIcons[category], { className: "w-4 h-4" })}
                                    <span className='text-sm'>{category}</span>
                                </div>

                                {activeCategory === category && (
                                    <div className='absolute inset-0 rounded-full bg-primary blur-xl opacity-20 z-0' />
                                )}
                            </button>
                        ))}
                    </div>
                ...
                <FadeIn delay={200}>
                    <div className='relative '>
                        ...
                        {/* Navigation Arrows - Visible if more than 3 projects */}
                        {filterProjects.length > 3 && (
                            <>
                                <button
                                    onClick={preSlide}
